...
$ambiltransaksi = $koneksi->query("SELECT * FROM pembelian JOIN pengguna ON pembelian. id=pengguna.id order by pembelian.tanggalbeli desc, pembelian.idbeli desc");
while ($list = $ambiltransaksi->fetch_assoc()) {
	if ($list['metodepembayaran'] != 'COD') {
		if ($list['statusbeli'] != '') {
			$status = \Midtrans\Transaction::status($list['notransaksi']);
			$transaction = $status->transaction_status;
			if ($list['statusbeli'] == 'pending') {
				$koneksi->query("UPDATE pembelian SET statusbeli='$transaction' WHERE idbeli='$list[idbeli]' and statusbeli != 'Selesai'") or die(mysqli_error($koneksi));
			}
		}
	}
}
							...
							?>
								<tr>
									...
									<td>
										...
										<a class="btn btn-success btn-sm btn-round mt-2" target="_blank" href="notacetak.php?id=<?= $pecah['idbeli'] ?>">Nota</a>
